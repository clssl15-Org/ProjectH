...
public class TransAI : MonsterBase
{
    ...
    IEnumerator IdleDelayCoroutine()
    {
        ...
        while (elapsed < waitTime)
        {
            ...
            if (!PlayerDetected() && currentState != State.Attack)
            {
                ChangeState(State.Run);
                isIdleDelay = false;
                yield break;
            }
            if (!PlayerInAttackRange() && currentState != State.Attack)
            {
                ChangeState(State.Detect);
                isIdleDelay = false;
                yield break;
