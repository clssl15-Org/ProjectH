...
public class CrowProjectile : KinematicProjectile
{
    ...
    public override void OnArrived()
    {
        ...
        bool TryGetImpactTime(out float impactTime)
        {
            ...
            {
                Debug.LogWarning(
                    Ctx($"{nameof(impactPrefab)}이(가) Animator 컴포넌트를 가지고 있지 않기 때문에 Impact 애니메이션을 재생하지 않습니다."));
            ...
            {
                Debug.LogWarning(
                    Ctx($"[CrowProjectile] {nameof(impactPrefab)}의 애니메이터가 {impactAnimName} 애니메이션을 가지고 있지 않기 때문에 Impact 애니메이션을 재생하지 않습니다."));
