...
public class DokebiAI : MonsterBase
{
    ...
    private bool isLAttacked = false; // 땅 공격 사용 여부 확인
    ...
    bool PlayerInAttackRange()
    {
        ...
        if (Vector2.Distance(transform.position, playerTransform.position) <= attackRange + 0.2f &&
            Vector2.Distance(transform.position, playerTransform.position) >= attackRange - 0.2f)
        {
            return true;
        }
        else return false;
