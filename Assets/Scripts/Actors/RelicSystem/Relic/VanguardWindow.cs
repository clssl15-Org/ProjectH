...
public class VanguardWindow : Relic, IRelicPlayerRebindHandler
{
    ...
    private static RushStabbing GetSkill(Player player)
    {
        if (player == null)
            throw new System.ArgumentNullException(nameof(player));

        if (player.StatesGO == null)
            throw new System.InvalidOperationException("[VanguardWindow] 플레이어의 StatesGO가 할당되지 않았습니다.");

        var skill = player.StatesGO.GetComponent<RushStabbing>();
        if (skill == null)
            throw new System.InvalidOperationException("[VanguardWindow] RushStabbing 컴포넌트를 찾을 수 없습니다.");
